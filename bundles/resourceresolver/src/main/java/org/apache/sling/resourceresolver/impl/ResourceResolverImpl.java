...
public class ResourceResolverImpl extends SlingAdaptable implements ResourceResolver {
    ...
    private String activeMappingService;
    ...
    throws LoginException {
        ...
        if(!authenticationInfo.containsKey("mappingService")) {
            authenticationInfo.put("mappingService", "default");
        }
        final ResourceResolverControl control = new ResourceResolverControl(isAdmin, authenticationInfo, resourceProviderTracker);
        this.activeMappingService = (String) authenticationInfo.get("mappingService");
    ...
    private Resource resolveInternal(final HttpServletRequest request, String absPath) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            final Iterator<MapEntry> mapEntriesIterator = this.factory.getMapEntries(activeMappingService).getResolveMapsIterator(requestPath);
    ...
    public String map(final HttpServletRequest request, final String resourcePath) {
        ...
        if (res != null) {
            ...
            while (path != null) {
                ...
                if (current != null && !path.endsWith(JCR_CONTENT_LEAF)) {
                    if (factory.getMapEntries(activeMappingService).isOptimizeAliasResolutionEnabled()) {
                        logger.debug("map: Optimize Alias Resolution is Enabled");
                        String parentPath = ResourceUtil.getParent(path);
                        if (parentPath != null) {
                            final Map<String, String> aliases = factory.getMapEntries(activeMappingService).getAliasMap(parentPath);
        ...
        for (final MapEntry mapEntry : this.factory.getMapEntries(activeMappingService).getMapMaps()) {
    ...
    private Resource getChildInternal(final Resource parent, final String childName) {
        ...
        if (factory.getMapEntries(activeMappingService).isOptimizeAliasResolutionEnabled()){
            ...
            final Map<String, String> aliases = factory.getMapEntries(activeMappingService).getAliasMap(parent.getPath());
            if (aliases != null) {
